...
export default class SystemInfo extends GObject.Object {
    ...
    @property(Number)
    get cpu_usage() { return this.#cpuUsage }

    @property(Number)
    get mem_usage() { return this.#memUsage }

    constructor() {
        super()

        if (!SystemInfo.cpu) 
            SystemInfo.cpu = new GTop.glibtop_cpu();

        if (!SystemInfo.memory)
            SystemInfo.memory = new GTop.glibtop_mem();
